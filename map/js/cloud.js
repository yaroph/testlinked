...
function bindCloudActionButton(button, handler) {
    return bindAsyncActionButton(button, handler);
}
...
function renderCloudHomeLoading(localPanel = 'cloud', note = 'Chargement du cloud...') {
    const activePanel = localPanel === 'local' ? 'local' : 'cloud';
    const shell = `
        <div class="cloud-home-loading-shell">
            <div class="cloud-home-loading-head">
                <div class="cloud-home-loading-tabs">
                    <button type="button" class="mini-btn" disabled style="opacity:${activePanel === 'cloud' ? '1' : '0.55'};">Cloud</button>
                    <button type="button" class="mini-btn" disabled style="opacity:${activePanel === 'local' ? '1' : '0.55'};">Local</button>
                </div>
            </div>
            <div class="cloud-home-loading-note">${escapeHtml(note)}</div>
            <div class="cloud-home-loading-stack">
            ...
            <div class="data-hub-status cloud-home-loading-status">
                <span class="data-hub-status-pill">${collab.user ? `Connecte: ${escapeHtml(collab.user.username || '')}` : 'Invite'}</span>
                <span class="data-hub-status-pill">${getCloudModalStatusLabel()}</span>
            </div>
        </div>
    `;
    openCloudModal('CLOUD COLLABORATIF', shell, '');
}
...
function renderCloudMembersLoading() {
    openCloudModal(
        'GESTION BOARD',
        `
            <div class="cloud-manage-loading-shell">
                <div class="cloud-manage-loading-note">Chargement des acces et des membres...</div>
                <div class="cloud-home-loading-stack">
                    <div class="cloud-loading-card">
                        <div class="cloud-loading-bar cloud-loading-bar-lg"></div>
                        <div class="cloud-loading-bar"></div>
